...
export default function useMakers(userLocation) {
  ...
  const fetchMakers = useCallback(async () => {
    ...
    const [makersResult, popResult] = await Promise.all([
      supabase
        .from("makers")
        .select("id, slug, name, bio, category, city, address, country, lat, lng, hero_color, avatar_url, gallery_urls, website_url, instagram_handle, opening_hours, is_featured, is_spotlight, spotlight_quote, is_verified, made_in_ireland, years_active, events"),
