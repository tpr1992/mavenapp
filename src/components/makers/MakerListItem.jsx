...
export default memo(function MakerListItem({ maker, index, isSaved, onTap, onToggleSave, showHours = true, highlightQuery }) {
    ...
    >
      <div style={{ position: "relative", flexShrink: 0 }}>
        ...
        {maker.made_in_ireland && (
          <svg width="16" height="11" viewBox="0 0 20 14" style={{
            position: "absolute",
            bottom: -1,
            right: -3,
            borderRadius: 1,
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        >
          ...
          {" \u00B7 "}{maker.category}
          {showHours && <> {"\u00B7"} {getTodayHours(maker.opening_hours)}</>}
          {!showHours && maker.distance != null && <> {"\u00B7"} {maker.city}</>}
        </div>
      </div>
      {onToggleSave && (
        <button
