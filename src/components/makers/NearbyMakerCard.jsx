...
export default memo(function NearbyMakerCard({ maker, onTap }) {
  ...
  return (
    <div
      ...
      onPointerMove={(e) => {
        if (!touch.current.moved && (Math.abs(e.clientX - touch.current.x) > 8 || Math.abs(e.clientY - touch.current.y) > 8))
          touch.current.moved = true
      }}
      onClick={() => { if (!touch.current.moved) onTap(maker) }}
      style={{
        minWidth: 180,
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div style={{ minWidth: 0 }}>
          ...
          >
            {maker.category} {"\u00B7"} {maker.distance != null ? formatDistance(maker.distance) : maker.city}
          </div>
        </div>
      </div>
    </div>
