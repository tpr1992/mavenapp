...
export default function LocationPicker({ userLocation, locationLabel, locationSource, setLocation, onClose }) {
  ...
  const handleUseCurrentLocation = useCallback(async () => {
    ...
    if (navigator.permissions) {
      try {
        ...
        if (status.state === "denied") {
          setCityError("Location is blocked for this site. Tap the lock icon in your address bar, then allow Location and reload the page.")
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        setCityError(
          err.code === 1 ? "Location is blocked for this site. Tap the lock icon in your address bar, then allow Location and reload the page."
  ...
  const handleSelectCity = useCallback((town) => {
          ...
          <div style={{ borderTop: `1px solid ${theme.border}` }}>
            ...
            >
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ flexShrink: 0 }}>
                ...
                <circle cx="7" cy="7" r="6" stroke={isGps ? theme.text : theme.textMuted} strokeWidth="1.2" fill="none" />
                <line x1="7" y1="0" x2="7" y2="2.5" stroke={isGps ? theme.text : theme.textMuted} strokeWidth="1.2" />
                <line x1="7" y1="11.5" x2="7" y2="14" stroke={isGps ? theme.text : theme.textMuted} strokeWidth="1.2" />
                <line x1="0" y1="7" x2="2.5" y2="7" stroke={isGps ? theme.text : theme.textMuted} strokeWidth="1.2" />
                <line x1="11.5" y1="7" x2="14" y2="7" stroke={isGps ? theme.text : theme.textMuted} strokeWidth="1.2" />
              </svg>
              <span style={{
                fontFamily: "'DM Sans', sans-serif",
                fontSize: 14,
              ...
              {isGps && (
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ marginLeft: "auto", flexShrink: 0 }}>
                  <path d="M2.5 7L5.5 10L11.5 4" stroke={theme.text} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              )}
            </div>
            {cityError && (
              ...
              {citySuggestions.map((town, i) => (
                <div
                  ...
                  style={{
                    padding: "13px 20px",
                    cursor: "pointer",
                    borderBottom: i < citySuggestions.length - 1 ? `1px solid ${theme.border}` : "none",
                    display: "flex",
                    alignItems: "center",
                    gap: 8,
