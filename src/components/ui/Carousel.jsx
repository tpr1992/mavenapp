...
export const TRANSITION_SPRING = { type: "spring", stiffness: 180, damping: 22, mass: 1.0 }
...
const Carousel = memo(function Carousel({
  ...
  const loopIdxToReal = useCallback((loopIdx) => {
  ...
  const slideToLoopIndex = useCallback((loopIdx) => {
    ...
    animCtrl.current = motionAnimate(scrollMV, loopIdx * el.offsetWidth, {
      ...
      onComplete: () => {
        ...
        if (idx < CLONES || idx >= CLONES + count) {
          ...
          scrollMV.set(el.scrollLeft, false)
          el.style.scrollSnapType = "x mandatory"
        }
      },
    })
  ...
  const updateMiniDots = useCallback((realIdx) => {
    const dotsEl = dotsRef.current
    if (!dotsEl) return
    for (let i = 0; i < count; i++) {
      dotsEl.children[i].style.opacity = i === realIdx ? "1" : "0.5"
    }
  }, [count])
  ...
  const handleTouchStart = useCallback(() => {
    ...
    clearTimeout(settleTimer.current)
    if (autoPlay) clearInterval(intervalRef.current)
  }, [autoPlay])
