    ...
    <div onClick={() => onTap(maker)} style={{ padding: "0 4px", cursor: "pointer" }}>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 16 }}>
          ...
          <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: "rgba(255,255,255,0.6)" }}>
            {maker.distance != null ? `\u25C8 ${maker.distance} km away` : `\u25C8 ${maker.city}`}
          </span>
          {showOpenStatus && (
            <span style={{
              fontFamily: "'DM Sans', sans-serif",
...
const FeaturedCarousel = memo(function FeaturedCarousel({ makers, onTap, showOpenStatus }) {
  ...
  return (
    <Carousel
      items={makers}
      renderItem={(maker) => <FeaturedCard maker={maker} onTap={onTap} showOpenStatus={showOpenStatus} isDark={isDark} />}
      keyExtractor={(m) => m.id}
      loop
      dots="pill"
    />
    ...
    <div onClick={() => onTap(maker)} style={{ padding: "0 4px", cursor: "pointer" }}>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 12, marginTop: 16 }}>
          <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 12, lineHeight: 1, color: "rgba(255,255,255,0.6)" }}>
            {CATEGORY_EMOJI[maker.category]} {maker.category}
          </span>
          <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 12, lineHeight: 1, color: "rgba(255,255,255,0.6)" }}>
...
const TrendingCarousel = memo(function TrendingCarousel({ makers, onTap, showOpenStatus }) {
  ...
  return (
    <Carousel
      items={makers}
      renderItem={(maker) => <TrendingCard maker={maker} onTap={onTap} showOpenStatus={showOpenStatus} isDark={isDark} />}
      keyExtractor={(m) => m.id}
      loop
      autoPlay={7000}
...
const MasonryGrid = memo(function MasonryGrid({ allMakers, visibleIds, sponsoredPosts, savedIds, onMakerTap, onToggleSave, theme, showOpenStatus }) {
  const hasAnimated = useRef(false)
  const [filterTick, setFilterTick] = useState(0)
  const prevVisible = useRef(visibleIds)
  useEffect(() => { const t = setTimeout(() => { hasAnimated.current = true }, 600); return () => clearTimeout(t) }, [])
  useEffect(() => {
  ...
  const tapProps = (fn) => ({
    onPointerDown,
    onPointerMove,
    onClick: () => { if (!touchRef.current.moved) fn() },
  })
      ...
      >
        ...
        <div style={{ padding: "8px 10px 9px", minWidth: 0, overflow: "hidden" }}>
          ...
          <div style={{
            ...
            {maker.distance != null ? `${formatDistance(maker.distance)} · ${maker.city}` : maker.city}
          </div>
        </div>
      </motion.div>
    )
    ...
    <div style={{ padding: "0 4px" }}>
      <div style={{ display: "flex", gap: 6, alignItems: "flex-start" }}>
        {columns.map((colItems, col) => (
          <div key={col} style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6, minWidth: 0, marginTop: 0 }}>
            {(() => {
              let visibleCount = 0
              const deferred = []
              const result = []
...
export default function DiscoverScreen({ makers = [], makersLoading, makersError, onRetry, onRefresh, onMakerTap, savedIds, onToggleSave, onScrollToTop, scrollContainerRef, locationLabel, locationSource, userLocation, setLocation, sponsoredPosts = [] }) {
  ...
  const allFiltered = useMemo(() => makers
    .filter((m) => category === "All" || m.category === category.toLowerCase())
    .filter((m) => !openNow || isOpenNow(m.opening_hours))
    .filter((m) => !q || m.name.toLowerCase().includes(q) || m.category.toLowerCase().includes(q) || m.city.toLowerCase().includes(q)),
  ...
  return (
    ...
    >
      <Helmet>
        ...
        <meta name="description" content="Discover local craftspeople and makers in Galway, Ireland. Find ceramicists, woodworkers, jewellers, and more near you." />
      </Helmet>
      ...
      <div style={{
        ...
        transition: "transform 0.4s cubic-bezier(0.32, 0.72, 0, 1), opacity 0.3s ease, border-color 0.4s ease",
        ...
        <div style={{
          ...
          <h2
            onClick={() => { onScrollToTop(); setCategory("All"); setSearchQuery(""); setCompactSearchOpen(false); setSearchOpen(false) }}
            style={{
              fontFamily: "'Playfair Display', serif",
              fontSize: 18,
              fontWeight: 700,
          ...
          <button
            ...
            onClick={() => {
              ...
              setTimeout(() => { if (compactSearchRef.current) compactSearchRef.current.focus() }, 50)
            }}
            style={{
              width: 28,
              height: 28,
          ...
          >
            <svg width="12" height="12" viewBox="0 0 16 16" fill="none" style={{
              ...
              <line x1="11" y1="11" x2="14.5" y2="14.5" stroke="#777" strokeWidth="1.6" strokeLinecap="round" />
            </svg>
            <svg width="11" height="11" viewBox="0 0 10 10" fill="none" style={{
              ...
              <line x1="2" y1="2" x2="8" y2="8" stroke="#fff" strokeWidth="1.5" strokeLinecap="round" />
              <line x1="8" y1="2" x2="2" y2="8" stroke="#fff" strokeWidth="1.5" strokeLinecap="round" />
            </svg>
          </button>
        </div>
        <div style={{
            ...
            <div style={{
              ...
              <div style={{
                ...
                <svg width="14" height="14" viewBox="0 0 16 16" fill="none" style={{ flexShrink: 0, opacity: 0.4 }}>
                  ...
                  <line x1="11" y1="11" x2="14.5" y2="14.5" stroke="#1a1a1a" strokeWidth="1.5" strokeLinecap="round" />
                </svg>
                <input
                ...
                {searchQuery && (
                  ...
                  >
                    <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                      <line x1="2.5" y1="2.5" x2="7.5" y2="7.5" stroke={theme.textMuted} strokeWidth="1.4" strokeLinecap="round" />
                      <line x1="7.5" y1="2.5" x2="2.5" y2="7.5" stroke={theme.textMuted} strokeWidth="1.4" strokeLinecap="round" />
                    </svg>
                  </button>
                )}
              </div>
                ...
                {makerSuggestions.map((maker, i) => (
                  <div
                    ...
                    style={{
                      padding: "10px 14px",
                      cursor: "pointer",
                      borderBottom: i < makerSuggestions.length - 1 ? `1px solid ${theme.border}` : "none",
                      display: "flex",
                      alignItems: "center",
                      gap: 10,
                    }}
                  >
                    <span style={{ fontSize: 14, flexShrink: 0 }}>{CATEGORY_EMOJI[maker.category]}</span>
      ...
      <div ref={fullHeaderRef}>
        <div style={{ padding: "12px 16px 14px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
              <h1
                onClick={() => { onScrollToTop(); setSearchQuery(""); setSearchOpen(false) }}
                style={{
                  fontFamily: "'Playfair Display', serif",
                  fontSize: 30,
                  fontWeight: 700,
              ...
              >
                {locationSource === "gps" ? (
                  ...
                  <svg width="11" height="11" viewBox="0 0 16 16" fill="none" style={{ flexShrink: 0 }}>
                    <path d="M8 1.5C5.5 1.5 3.5 3.5 3.5 6c0 3.5 4.5 8.5 4.5 8.5s4.5-5 4.5-8.5c0-2.5-2-4.5-4.5-4.5zm0 6.25a1.75 1.75 0 110-3.5 1.75 1.75 0 010 3.5z" fill={locationLabel ? theme.textSecondary : theme.textMuted} />
                  </svg>
                )}
                <span
                  style={{
                ...
                <svg width="10" height="10" viewBox="0 0 10 10" fill="none" style={{ flexShrink: 0 }}>
                  <path d="M2.5 3.75L5 6.25L7.5 3.75" stroke={theme.textMuted} strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
            </div>
            ...
            <div style={{ position: "relative", zIndex: 10, marginTop: 8 }}>
              <div style={{
                ...
                {makerSuggestions.map((maker, i) => (
                  ...
                  >
                    <span style={{ fontSize: 15, flexShrink: 0 }}>{CATEGORY_EMOJI[maker.category]}</span>
                    <div style={{ minWidth: 0, flex: 1 }}>
                      <span style={{
                        fontFamily: "'DM Sans', sans-serif",
                        fontSize: 13.5,
        ...
        <CategoryPills selected={category} onSelect={setCategory} showOpenNow openNowActive={openNow} onToggleOpenNow={() => setOpenNow((v) => !v)} />
      </div>
          ...
          <div style={{
            ...
            >
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                <rect x="0" y="0" width="6" height="6" rx="1.5" fill={viewMode === "gallery" ? theme.text : theme.textMuted} />
                <rect x="8" y="0" width="6" height="6" rx="1.5" fill={viewMode === "gallery" ? theme.text : theme.textMuted} />
                <rect x="0" y="8" width="6" height="6" rx="1.5" fill={viewMode === "gallery" ? theme.text : theme.textMuted} />
                <rect x="8" y="8" width="6" height="6" rx="1.5" fill={viewMode === "gallery" ? theme.text : theme.textMuted} />
              </svg>
            </button>
            <button
            ...
            >
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                <rect x="0" y="1" width="14" height="2.5" rx="1" fill={viewMode === "list" ? theme.text : theme.textMuted} />
                <rect x="0" y="5.75" width="14" height="2.5" rx="1" fill={viewMode === "list" ? theme.text : theme.textMuted} />
                <rect x="0" y="10.5" width="14" height="2.5" rx="1" fill={viewMode === "list" ? theme.text : theme.textMuted} />
              </svg>
            </button>
          </div>
        </div>
