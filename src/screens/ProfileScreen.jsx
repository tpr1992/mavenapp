...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    ...
    supabase.from("profiles").select("first_name").eq("id", user.id).single()
      .then(({ data }) => { if (data?.first_name) setProfileName(data.first_name) })
  }, [user])

  const SETTINGS_ITEMS = [
    { icon: "\u25D4", label: "Notifications", subtitle: "Coming soon", action: () => {} },
    { icon: "\u25C7", label: "About maven", action: () => setShowAbout(true) },
    { icon: "\u2197", label: "Suggest a Maker", action: () => window.open("mailto:hello@maven.ie?subject=Maker%20Suggestion", "_blank", "noopener,noreferrer") },
  ]
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.error) {
      ...
      const friendly = mode === "signup" && raw.includes("already registered")
      ...
      if (mode === "signup") {
        ...
        if (currentUser) {
          const { data: profile } = await supabase.from("profiles").select("first_name").eq("id", currentUser.id).single()
    ...
    <div style={{ paddingBottom: 100 }}>
      ...
      <div style={{ padding: "16px 20px 20px" }}>
        ...
        {loading ? (
          ...
          >
            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 14, color: theme.textMuted, margin: 0 }}>
              Loading...
            </p>
          </div>
        ) : user ? (
        ...
        >
          <span style={{ fontSize: 18, color: theme.textMuted, width: 24, textAlign: "center" }}>{"\u2600"}</span>
          <span
            style={{
              fontFamily: "'DM Sans', sans-serif",
              fontSize: 14.5,
        ...
        {SETTINGS_ITEMS.map((item, i) => (
          ...
          >
            <span style={{ fontSize: 18, color: theme.textMuted, width: 24, textAlign: "center" }}>{item.icon}</span>
            <div style={{ flex: 1 }}>
              <span
                style={{
                  fontFamily: "'DM Sans', sans-serif",
        ...
        >
          ...
          >
            ...
            <p style={{
              ...
              Discover local makers and craftspeople in Galway, Ireland. Supporting the people who make things by hand.
            </p>
            <button
              onClick={() => setShowAbout(false)}
              style={{
